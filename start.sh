...
echo "📚 Генерируем документацию API с помощью Scribe..."
if ! docker-compose exec -T php php artisan scribe:generate; then
    echo "⚠️ Ошибка при генерации документации Scribe (продолжаем выполнение)"
fi
echo "✅ Документация Scribe сгенерирована (если установлена)"
