...
help:
	...
	@echo "  install    - Установить зависимости (Composer, NPM)"
	...
	@echo "  npm        - Выполнить команду NPM (например, make npm COMMAND='install')"
...
# Интерактивная консоль в PHP контейнере
shell:
	@echo "=== Вход в контейнер PHP ==="
...
artisan:
	@if [ -z "${COMMAND}" ]; then \
		...
		echo "Использование: make artisan COMMAND='command args'"; \
		echo "Пример: make artisan COMMAND='migrate'"; \
	else \
		$(DOCKER_COMPOSE) run --rm $(PHP_CONTAINER) php artisan $(COMMAND); \
	fi
...
npm:
	@if [ -z "${COMMAND}" ]; then \
		echo "Ошибка: не указана команда"; \
		echo "Использование: make npm COMMAND='command args'"; \
		echo "Пример: make npm COMMAND='run build'"; \
	else \
		cd backend && npm $(COMMAND); \
	fi
...
scribe:
	@echo "=== Генерация документации API с помощью Scribe ==="
	$(DOCKER_COMPOSE) run --rm $(PHP_CONTAINER) php artisan scribe:generate
	@echo "Документация Scribe сгенерирована! Откройте http://localhost:8000/docs в браузере."
